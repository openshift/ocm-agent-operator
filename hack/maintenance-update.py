#!/usr/bin/env python3
"""
Maintenance update script to automate common tasks for maintenance updates
across different versions of OCP for ocm-agent-operator.

Ported from managed-upgrade-operator (SREP-915) and adapted for
ocm-agent-operator dependency structure.
"""
...
def show_spinner(duration=1):
    ...
    for i in range(duration * 10):
        ...
        print(f"\r{Colors.CYAN}{char}{Colors.RESET} Processing...",
              end='', flush=True)
        time.sleep(0.1)
    print("\r" + " " * 20 + "\r", end='')
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Maintenance update script to automate common tasks for "
            "maintenance updates across different versions of OCP"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python maintenance-update.py
      Run without release version (latest deps + boilerplate only)

  python maintenance-update.py --release release-4.19
      Run with release flag (version deps + latest deps + boilerplate)

  python maintenance-update.py --release release-4.19 \\
      --version-deps github.com/openshift/api github.com/custom/dep
      Run with custom version dependencies

  python maintenance-update.py \\
      --latest-deps github.com/openshift/osde2e-common
      Run with custom latest dependencies
        """
    )
    ...
    parser.add_argument(
        '--version-deps',
        nargs='*',
        default=['github.com/openshift/api'],
        help=(
            'List of dependencies to update with release version '
            '(default: github.com/openshift/api)'
        )
    )
    ...
    parser.add_argument(
        '--latest-deps',
        nargs='*',
        default=['github.com/openshift/osde2e-common'],
        help=(
            'List of dependencies to update to latest version '
            '(default: github.com/openshift/osde2e-common)'
        )
    )
...
def run_command(command, capture_output=False, check=True,
    ...
    try:
        if capture_output:
            ...
            result = subprocess.run(
                command, shell=isinstance(command, str), check=check)
...
def validate_release_version(release_version):
    ...
    if not release_version.startswith('release-'):
        logger.error(
            "Release version must start with 'release-' "
            "(e.g., release-4.19)")
        sys.exit(1)
    ...
    try:
        ...
        logger.error(
            f"Invalid release version format: {release_version}")
        sys.exit(1)
...
def update_version_based_dependencies(release_version, version_deps):
    ...
    print_section(
        f"Updating Version Dependencies ({release_version})")

    filtered_deps = [dep for dep in version_deps if dep.strip()]
    if not filtered_deps:
        print_info(
            "No version dependencies specified, "
            "skipping version-based updates")
        return
    ...
    print()
    dependencies = [
        f"{dep}@{release_version}" for dep in filtered_deps]
    ...
    print_success(
        f"Updated {len(dependencies)} version-based dependencies")

    # Update controller-runtime based on the release version
    print_section("Updating Controller Runtime")
    controller_runtime_versions = {
        'release-4.17': 'v0.18.0',
        'release-4.18': 'v0.19.0',
        'release-4.19': 'v0.20.0',
        'release-4.20': 'v0.21.0',
    }

    if release_version in controller_runtime_versions:
        cr_version = controller_runtime_versions[release_version]
        dep = f"sigs.k8s.io/controller-runtime@{cr_version}"
        print_step(1, 1, f"Updating {dep}")
        run_command(['go', 'get', dep])
        print_success("Controller runtime updated successfully")
    else:
        print_warning(
            f"No controller-runtime mapping for: {release_version}")
        print_info(
            f"Supported versions: "
            f"{', '.join(controller_runtime_versions.keys())}")
        print_info(
            "Skipping controller-runtime update. "
...
def update_latest_dependencies(latest_deps):
    ...
    if not latest_deps or (
            len(latest_deps) == 1 and not latest_deps[0].strip()):
        print_info(
            "No latest dependencies specified, "
            "skipping latest updates")
        return
...
def show_git_diff():
    ...
    run_command(
        ['git', 'status', '--porcelain'], show_command=False)
...
def prompt_for_commit(repo_root, commit_message):
    ...
    try:
        ...
        print(
            f"Default message: "
            f"{Colors.CYAN}\"{commit_message}\"{Colors.RESET}")
        confirm = input(
            f"\n{Colors.BOLD}Do you wish to commit the changes? "
            f"(y/yes/n/No):{Colors.RESET} ").strip().lower()

        if confirm in ['y', 'yes']:
            print(
                f"\n{Colors.YELLOW}"
                f"Custom Commit Message{Colors.RESET}")
            print(
                "Press Enter to use default message, "
                "or type your custom message:")

            custom_message = input(
                f"{Colors.BOLD}Commit message:"
                f"{Colors.RESET} ").strip()

            final_message = (
                custom_message if custom_message else commit_message)

            print_info(
                f"Committing with message: \"{final_message}\"")
            run_command(['git', 'commit', '-m', final_message])
            print_success("Changes committed successfully!")
        else:
            print_warning(
                "Not committing the changes to repository. "
                "Exiting.")
            sys.exit(1)
...
def prompt_for_add(repo_root):
    ...
    if staged_files:
        staged_list = [
            f for f in staged_files.split('\n') if f.strip()]
        print_success(
            f"Already staged ({len(staged_list)} files):")
    ...
    try:
        for i, file in enumerate(files, 1):
            print(
                f"\n{Colors.CYAN}[{i}/{len(files)}]"
                f"{Colors.RESET} {file}")
            confirm = input(
                f"{Colors.BOLD}Add this file to git cache? "
                f"(y/yes/n/No/a/all):{Colors.RESET} "
            ).strip().lower()
            ...
            if confirm in ['y', 'yes']:
                ...
                print_success(
                    "Adding all remaining files to git cache")
                remaining_files = files[i-1:]
                for remaining_file in remaining_files:
                    run_command(
                        ['git', 'add', remaining_file],
                        show_command=False)
                print_info(
                    f"Added {len(remaining_files)} files "
                    f"to git cache")
                break
            else:
...
def handle_git_changes(repo_root, commit_message,
                       should_commit=True):
    """
    ...
    Args:
        ...
        commit_message: Default commit message to use
        should_commit: Whether to prompt for commit (True)
            or just stage files (False)
    """
    if not has_git_changes():
        print_success(
            "No git changes detected - repository is clean")
        return

    print_warning(
        "Found local changes in the git repository")
    ...
    if should_commit:
        ...
        print_info(
            ...
            "files have been staged only")
        print_warning(
            "Remember to commit your changes manually "
            "when ready")
...
def run_make_tests(repo_root, validate_message):
    ...
    container_make_path = os.path.join(
        repo_root, 'boilerplate', '_lib', 'container-make')

    print_info(
        f"Using container-make script: {container_make_path}")
...
def main():
    ...
    print(
        f"  {Colors.CYAN}Release version:{Colors.RESET} "
        f"{release_version or 'None'}")
    print(
        f"  {Colors.CYAN}Version dependencies:{Colors.RESET} "
        f"{len(args.version_deps)} items")
    print(
        f"  {Colors.CYAN}Latest dependencies:{Colors.RESET} "
        f"{len(args.latest_deps)} items")
    ...
    if release_version:
        ...
        update_version_based_dependencies(
            release_version, args.version_deps)

    # Update latest dependencies
    print_step(1, 1, "Updating latest dependencies...")
    update_latest_dependencies(args.latest_deps)

    # Tidy up dependencies before validations
    ...
    run_make_tests(
        repo_root, "Validate go.mod update changes")
    ...
    run_make_tests(
        repo_root, "Validate boilerplate update changes")

    # Handle git changes after validation for boilerplate
    handle_git_changes(
        repo_root,
        commit_message="Updating boilerplate dependencies",
        should_commit=True)
    ...
    print(
        ...
        f"All maintenance tasks completed successfully!"
        f"{Colors.RESET}\n")
